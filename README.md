...
> 当前版本为 **1.1.2**。项目只发布 Windows x64 安装版桌面应用，
> 不发布网页版或 Portable 版。
...
- 双练习模式：可在「挖空」和「整题」之间切换。
- 一屏答题：复习页占满整个窗口高度，题干、进度和「不会、不熟、熟练」按钮始终同屏可见，
  卡片高度固定，翻题时按钮位置不会跳动；只有超出卡片的长题在题干区域内滚动。
- 键盘操作：空格核对答案，再按 1、2、3 选择「不会、不熟、熟练」，快捷键直接印在按钮上。
...
```text
desktop-app-v1.1.2/829-memory-plan-1.1.2-setup.exe
```
